    ...
    public class TSharkInterop
    {
        ...
        public async Task<string[]> GetPacketsDescriptions(string path)
        {
            ...
            Command? command = CliWrap.Cli.Wrap(TsharkPath)
                                ...
                                .WithStandardOutputPipe(PipeTarget.ToStringBuilder(sb, Encoding.UTF8));
            CommandTask<CommandResult>? cliTask = command.ExecuteAsync();
            await cliTask.Task;
             ...
             List<string> toBeEnabledHeurs = null, List<string> toBeDisabledHeurs = null)
        {
            ...
            {
                // We are passing a pipe to TShark. This can lead to the following error if things are synchronized well:
                // ```
                // Capturing on '\\.\pipe\ps_2_ws_pipe1097115123'
                // tshark: The capture session on "\\.\pipe\ps_2_ws_pipe1097115123" could not be started due to error on pipe open: The system cannot find the file specified.  (2).
                // ```
                int x = 3;
        ...
        public Task<TSharkCombinedResults> GetPdmlAndJsonAsync(TempPacketSaveData packets,
            List<string> toBeEnabledHeurs = null, List<string> toBeDisabledHeurs = null)
            => GetPdmlAndJsonAsync(new TempPacketSaveData[] { packets }, 0, toBeEnabledHeurs, toBeDisabledHeurs);
        public Task<TSharkCombinedResults> GetPdmlAndJsonAsync(IEnumerable<TempPacketSaveData> packets, int packetIndex,
            List<string> toBeEnabledHeurs = null, List<string> toBeDisabledHeurs = null)
            => GetPdmlAndJsonAsync(packets, packetIndex, CancellationToken.None, toBeEnabledHeurs, toBeDisabledHeurs);
        public Task<TSharkCombinedResults> GetPdmlAndJsonAsync(IEnumerable<TempPacketSaveData> packets, int packetIndex,
            CancellationToken token, List<string> toBeEnabledHeurs = null, List<string> toBeDisabledHeurs = null)
        {
