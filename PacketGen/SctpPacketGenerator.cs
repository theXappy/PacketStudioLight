...
namespace PacketGen
{
    internal class SctpPacketGenerator
    {
        private static uint[] crc32c_table = new uint[] {
        ...
        {
            uint crc = 0xff_ff_ff_ff;
            int length = data.Length;
            int i = 0;
            while (length > 0)
            {
                length--;
                crc = crc32c_table[(crc ^ data[i++]) & 0xFFL] ^ (crc >> 8);
            }

            return ~crc;
        }

        public static uint Adler32(byte[] str)
        {
            const int mod = 65521;
            uint a = 1, b = 0;
            ...
            var res = (b << 16) | a;
            return (res >> 24) | ((res << 8) & 0x00FF0000) |
                   ((res >> 8) & 0x0000FF00) | (res << 24);
        }

        private enum ChecksumAlgo
        {
            Adler32,
            Crc32c
        }
        ...
        public static (Packet, LinkLayers) GenerateSctp(Dictionary<string, string> variables)
        {
            ...
            // Calc new Checksum field
            ChecksumAlgo checksumAlgo = ChecksumAlgo.Adler32;
            uint checksum = 0;
            if (variables.TryGetValue("SCTP_CHECKSUM_ALGO", out string checksumAlgoStr) &&
                checksumAlgoStr.ToUpperInvariant() == "CRC32C")
            {
                checksumAlgo = ChecksumAlgo.Crc32c;
            }


            if (variables.TryGetValue("SCTP_CHECKSUM", out string checksumOverride) &&
                uint.TryParse(checksumOverride, out checksum))
            {
            ...
            else
            {
                ...
                switch (checksumAlgo)
                {
                    case ChecksumAlgo.Crc32c:
                        checksum = Crc32c(sctpBytes);
                        break;
                    case ChecksumAlgo.Adler32:
                    default:
                        checksum = Adler32(sctpBytes);
                        break;
                }
            }

            byte[] checksumBytes = BitConverter.GetBytes(checksum);
            checksumBytes.CopyTo(sctpBytes, 8);

            GenericPacket sctpPacket = new GenericPacket(sctpBytes);

            (Packet ip, LinkLayers firstLayer) = PacketsGenerator.GenerateIp(variables);
            ip.PayloadPacket = sctpPacket;

            // Fix IP's next protocol
            IPv4Packet castedParent = (IPv4Packet)ip;
            castedParent.Protocol = (ProtocolType)132;
